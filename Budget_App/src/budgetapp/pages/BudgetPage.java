...
public class BudgetPage extends BaseFrame {
    ...
    protected void initUI() {
        ...
        buttonPanel.setBackground(new Color(242, 243, 247));
    ...
    private void saveBudgetData() {
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql = "INSERT INTO budget (user_id, month, income, needs_percent, wants_percent, savings_percent, budget_set) " +
                    "VALUES (?, ?, ?, ?, ?, ?, TRUE) " +
                    "ON DUPLICATE KEY UPDATE income = ?, needs_percent = ?, wants_percent = ?, savings_percent = ?, budget_set = TRUE";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setInt(1, userId);
                stmt.setString(2, "current_month");
    ...
    private void loadBudgetData() {
        try (Connection conn = DatabaseConnection.getConnection()) {
            String sql = "SELECT income, needs_percent, wants_percent, savings_percent FROM budget WHERE user_id = ? AND month = ?";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                stmt.setInt(1, userId);
                stmt.setString(2, "current_month");
